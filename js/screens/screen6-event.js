...
function renderEventDetailScreen(eventKey) {
  ...
  screen.innerHTML = `
    ...
    <div class="event-gallery-section">
      ...
      <div class="share-cards-row">
        <!-- WhatsApp Card -->
        <div class="share-card share-card-wa">
          ...
          <h3 class="share-card-title ${isHindi ? 'text-hindi' : ''}">${t('waSendPhotos')}</h3>
          <p class="share-card-subtext ${isHindi ? 'text-hindi' : ''}">${t('waSubtext')}</p>
          <span class="share-card-tag share-tag-green">${t('waTag')}</span>
          <button class="share-card-btn btn-whatsapp" onclick="window.open('${getWhatsAppUrl(eventWaName, lang)}', '_blank')">
            ${t('waOpenBtn')}
          </button>
        </div>

        <!-- Direct Upload Card -->
        <div class="share-card share-card-upload">
          <div class="share-card-icon">
            <svg viewBox="0 0 24 24" width="26" height="26" fill="#C9A84C">
              <path d="M19.35 10.04C18.67 6.59 15.64 4 12 4 9.11 4 6.6 5.64 5.35 8.04 2.34 8.36 0 10.91 0 14c0 3.31 2.69 6 6 6h13c2.76 0 5-2.24 5-5 0-2.64-2.05-4.78-4.65-4.96zM14 13v4h-4v-4H7l5-5 5 5h-3z"/>
            </svg>
          </div>
          <h3 class="share-card-title ${isHindi ? 'text-hindi' : ''}">${t('uploadVideosLabel')}</h3>
          <p class="share-card-subtext ${isHindi ? 'text-hindi' : ''}">${t('uploadVideosSubtext')}</p>
          <span class="share-card-tag share-tag-gold">${t('uploadVideosTag')}</span>
          <button class="share-card-btn btn-gold" onclick="handleUploadClick('${eventKey}')">
            ${t('uploadBtn')}
          </button>
        </div>
      </div>

      <p class="share-note-text ${isHindi ? 'text-hindi' : ''}">${t('shareNote')}</p>
